...
var WhiteNoiseProcessor = /*#__PURE__*/function (_AudioWorkletProcesso) {
  _inherits(WhiteNoiseProcessor, _AudioWorkletProcesso);
  var _super = _createSuper(WhiteNoiseProcessor);
  function WhiteNoiseProcessor() {
    _classCallCheck(this, WhiteNoiseProcessor);
    return _super.apply(this, arguments);
  }
  _createClass(WhiteNoiseProcessor, [{
    key: "process",
    value: function process(inputs, outputs, parameters) {
      ...
      output.forEach(function (channel, ch_i) {
        for (var i = 0; i < channel.length; i++) {
          channel[i] = input[i] * Math.abs(Math.sin(d * (.0001 * (ch_i + 1))) * (ch_i + 1)) * (
